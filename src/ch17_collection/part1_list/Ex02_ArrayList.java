...
//		fruitList.add("mango");		// 정적인 리스트, 추가 및 삭제 안됨

		// 프리미티브 타입은 Wrapper 클래스로 리스트를 생성해야함
		...
		list.add(3, "collection");			// 3번 인덱스 앞에 추가
		System.out.println(list);
		list.addAll(fruitList);				// 맨 뒤에 컬렉션 추가
		System.out.println(list);			// 추가하는 리스트값 뒤에 자동으로 추가
		...
		// 데이터 삭제
		list.remove(6);						// 6번 인덱스값 삭제
		list.remove("cherry");				// "체리" 엘리먼트 삭제
		System.out.println(list);
		numList.clear();					// 리스트 전체 엘리먼트 삭제
